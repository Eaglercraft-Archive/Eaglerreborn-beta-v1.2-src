//This example plugin generates 'scrolls', through the command ".makescroll content_here".
//It needs creative mode.
//New lines can be generated by using the | character.
PluginAPI.require("network"); //Require the network, we need to send creative inventory change packets (to ensure the item generates server side aswell)
PluginAPI.require("player"); //Require the player, we need to know whether or not they're in creative mode.
PluginAPI.addEventListener("sendchatmessage", function (event) { //When the player tries to send a chat message
  ...
  if ( //If
    ...
    if (!PluginAPI.player.capabilities.isCreativeMode) { //Check if player is in creative mode
      PluginAPI.displayToChat({ 
    ...
    var stack = PluginAPI.constructors.ItemStack({ //Construct an ItemStack consisting of 1 piece of paper
      itemId: PluginAPI.items.paper.getID(),
      amount: 1,
      itemDamage: 0,
    });
    ...
    PluginAPI.network.sendPacketCreativeInventoryAction({ //Send the packet to the server, telling it to at slot ___, place an ItemStack
